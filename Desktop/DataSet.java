...
public class DataSet {
	...
	private double[][] data;
	public DataSet(double[][] dat ) {
		data = dat;
	}
	
	public double[][] SVD(int k) {
		double[][] v = data;
		Matrix m = new Matrix(v, v.length, v[0].length);
		SingularValueDecomposition svd = new SingularValueDecomposition(m);
		Matrix u = svd.getU();
		Matrix s = svd.getS();
		for (int i = k; i < v.length && i < v[0].length; i++) {
			m.set(i, i, 0);
		}
		Matrix r = u.times(s);
		double[][] pcaed = r.getMatrix(0, v[0].length, 0, k).getArray();
		return pcaed;
	}
	...
	private static int[] parseRow (String str) {
		...
		while (true) {
			...
			String i = s.substring(s.indexOf(" "));
